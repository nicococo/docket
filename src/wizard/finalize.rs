...
//!
//! - `~/.config/docket/config.toml`             — `[global]` + `[layout]`
//! - `~/.config/docket/<kind>[@<instance>].toml` — one per assigned widget
//! - `~/.config/docket/llm.toml` — when the picked provider differs from
//!   what's on disk
//! - `~/.config/docket/credentials/<provider>_key.toml` for the active
...
/// Outlook inbox sitting in `~/.cache/docket/email/main/` would otherwise
...
fn render_wizard_toml(state: &WizardState, widget_id: &str, wd: &WizardDescriptor) -> String {
    let mut out = String::new();
    out.push_str(&format!(
        "# Generated by `docket --setup`. Hand-edit freely; the wizard preserves\n\
...
fn write_provider_key(provider: &crate::llm::LlmProviderDef, key: &str) -> Result<()> {
    ...
    let body = format!(
        "# {} API key. Generated by `docket --setup`.\n\
         # Get one at {}.\n\
         api_key = {}\n",
...
fn render_llm_toml(provider: &crate::llm::LlmProviderDef, existing: Option<&str>) -> String {
    ...
    let cache_capacity = prev_limits
        ...
        .unwrap_or(1024);
    format!(
        "# Generated by `docket --setup`. Edit freely; `--setup` re-runs\n\
